...
compa <- rbind(data.frame(filter(compa,name.char=="scenario_baseline"),
                          type="Total reported cases"),
               data.frame(filter(compa,name.char=="scenario_baseline"),
                          type="Newly reported cases"))
report <- rbind(data.frame(report,
                          type="Total reported cases"),
               data.frame(report,
                          type="Newly reported cases"))

compa[1:52,c(2,4,5)] <- cumsum(compa[1:52,c(2,4,5)])
compa[53:104,c(2,4,5)] <- cumsum(compa[53:104,c(2,4,5)])
compa[105:156,c(2,4,5)] <- cumsum(compa[105:156,c(2,4,5)])
report$value[1:52] <- cumsum(report$value[1:52])
...
ggplot()+
  ...
  labs(x= "",y = "Reported cases")+  ##,title = "Overall cases fitting"
  theme_bw()+
  scale_y_sqrt()+
...
ggsave(file=paste0("fitted.check.pdf"),width=13,height=5)
ggsave(file=paste0("fitted.check.tiff"),width=13,height=5)
#########
...
citys.pic$group<-paste0("b) Other 50 cities")
wuhan.pic$group<-paste0("a) Wuhan")
all.pic$group<-paste0("c) Total")

updated.pic <- rbind(filter(wuhan.pic,Type=="1)scenario_baseline"),
                     filter(citys.pic,Type=="1)scenario_baseline"),
                     filter(all.pic,Type=="1)scenario_baseline"))

ggplot ()+
  geom_line(data = updated.pic, aes(x=date, y=mean.1,group=type,color=type),size=1) +
  geom_ribbon(data= updated.pic, aes(x=date, ymin = lo.1, ymax = hi.1, fill = type, color=type),
  ...
  theme(plot.title = element_text(hjust = 0.5,size = 20, face = "bold"),
        ...
        axis.title.y=element_text(size=14),legend.text=element_text(size=13),legend.title = element_text(size=13),
        strip.text.x = element_text(size = 15))
ggsave(file=paste0("D:/RA-POLYU/lockdown/9-4/diff.sce.sim.tiff"),width=15,height=5)
...
ggplot()+
  ...
  theme(plot.title = element_text(hjust = 0.5,size = 20, face = "bold"),
        ...
        axis.title.y=element_text(size=14),legend.text=element_text(size=13),legend.title = element_text(size=13),
        strip.text.x = element_text(size = 15))
ggsave(file=paste0("D:/RA-POLYU/lockdown/9-4/reduce.long.tiff"),
       width=8,height=4)
...
ggplot()+
  ...
  theme(plot.title = element_text(hjust = 0.5,size = 20, face = "bold"),
        ...
        strip.text.x = element_text(size = 15))
ggsave(file=paste0("bed.table.tiff"),width=15,height=5)
